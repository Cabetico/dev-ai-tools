...
#### A1.
"""
Create an online platform for online coding interviews  the app should be able to do the following:
- Create a link and share it with candidates
- Allow everyone who connects to edit code in the code panel
- Show real-time updates to all connected users
- Support syntax highlighting for multiple languages
...
- create a folder inside hw2 named "coding-interview-app" inside it create the backend and frontend folders and put the code there
- implement Frontend with React + Vite
- implement Backend bases on the OpenAPI specs, for now use a mock database
...
#### A2.
"""
first shut down backend and frontend services
"""

"""
now create integration tests that check the interaction between client and server, and create a `README.md` file at the coding-interview-app folder level with the commands for running and testing the application
"""
...
"""
lets create a process to run both frontend and backend at the same team using concurrently 
"""
...
"""
add suport for syntax highlighting for JavaScript and Python in the code editor, comment about the library choice for it
"""
...
"""
Now for security reasons, I want to use WASM to execute the code only in the browser, implement this feature and comment about the library choose for the implementation
"""
...
A:
I used **Docker Compose** to orchestrate the application with the following services and base images:

1.  **Frontend**: `nginx:alpine` (Multi-stage build using `node:22-alpine` for building React assets).
2.  **Backend**: `node:22-alpine`.
3.  **Database**: `postgres:15-alpine`.

This setup allows serving the frontend via Nginx, running the backend API, and having a persistent PostgreSQL database, all connected via a dedicated network.
